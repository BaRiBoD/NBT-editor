...
public abstract class NBTTestCase extends TestCase {
    ...
    protected byte[] serialize(JTag<?> tag) {
    ...
    protected JTag<?> deserialize(byte[] data) {
    ...
    protected JTag<?> deserializeFromFile(String f) {
    ...
    protected <T> void invokeSetValue(JTag<T> tag, T value) {
        try {
    ...
    protected <T> T invokeGetValue(JTag<T> tag) {
        try {
            Class<?> c = tag.getClass();
            Method m;
            while (c != Object.class) {
